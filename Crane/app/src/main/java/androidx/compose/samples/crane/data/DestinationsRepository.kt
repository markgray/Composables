...
class DestinationsRepository @Inject constructor(
    ...
    /**
     * A local copy of the [List] of all of the [City] objects in our dataset.
     */
    @Suppress("MemberVisibilityCanBePrivate")
    val cities: List<City> = listCities

    /**
     * This is called by [DetailsViewModel] to get the [City] whose [City.name] matches the
     * [String] passed as an extra under the key [KEY_ARG_DETAILS_CITY_NAME] when the
     * [launchDetailsActivity] method launches the [DetailsActivity]. The [City] it finds
     * is returned as a [Result.Success] of [City] by the [DetailsViewModel.cityDetails]
     ...
     *
     * @param cityName the [City.name] we are to search the [cities]
     * [List] of [City] for.
     * @return the first [City] whose [City.name] matches our [String] parameter [cityName]
