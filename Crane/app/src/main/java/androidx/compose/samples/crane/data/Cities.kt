/*
 * Copyright 2020 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *     https://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

package androidx.compose.samples.crane.data

/**
 * The [City] object for Madrid, Spain.
 */
val MADRID: City = City(
    name = "Madrid",
    country = "Spain",
    latitude = "40.416775",
    longitude = "-3.703790"
)

/**
 * The [City] object for Naples, Italy.
 */
val NAPLES: City = City(
    name = "Naples",
    country = "Italy",
    latitude = "40.853294",
    longitude = "14.305573"
)

/**
 * The [City] object for Dallas, US.
 */
val DALLAS: City = City(
    name = "Dallas",
    country = "US",
    latitude = "32.779167",
    longitude = "-96.808891"
)

/**
 * The [City] object for Cordoba, Argentina.
 */
val CORDOBA: City = City(
    name = "Cordoba",
    country = "Argentina",
    latitude = "-31.416668",
    longitude = "-64.183334"
)

/**
 * The [City] object for Maldivas, South Asia.
 */
val MALDIVAS: City = City(
    name = "Maldivas",
    country = "South Asia",
    latitude = "1.924992",
    longitude = "73.399658"
)

/**
 * The [City] object for Aspen, Colorado.
 */
val ASPEN: City = City(
    name = "Aspen",
    country = "Colorado",
    latitude = "39.191097",
    longitude = "-106.817535"
)

/**
 * The [City] object for Bali, Indonesia.
 */
val BALI: City = City(
    name = "Bali",
    country = "Indonesia",
    latitude = "-8.3405",
    longitude = "115.0920"
)

/**
 * The [City] object for Big Sur, California.
 */
val BIGSUR: City = City(
    name = "Big Sur",
    country = "California",
    latitude = "36.2704",
    longitude = "-121.8081"
)

/**
 * The [City] object for Khumbu Valley, Nepal.
 */
val KHUMBUVALLEY: City = City(
    name = "Khumbu Valley",
    country = "Nepal",
    latitude = "27.9320",
    longitude = "86.8050"
)

/**
 * The [City] object for Rome, Italy.
 */
val ROME: City = City(
    name = "Rome",
    country = "Italy",
    latitude = "41.902782",
    longitude = "12.496366"
)

/**
 * The [City] object for Granada, Spain.
 */
val GRANADA: City = City(
    name = "Granada",
    country = "Spain",
    latitude = "37.18817",
    longitude = "-3.60667"
)

/**
 * The [City] object for Washington DC, USA.
 */
val WASHINGTONDC: City = City(
    name = "Washington DC",
    country = "USA",
    latitude = "38.9072",
    longitude = "-77.0369"
)

/**
 * The [City] object for Barcelona, Spain.
 */
val BARCELONA: City = City(
    name = "Barcelona",
    country = "Spain",
    latitude = "41.390205",
    longitude = "2.154007"
)

/**
 * The [City] object for Crete, Greece.
 */
val CRETE: City = City(
    name = "Crete",
    country = "Greece",
    latitude = "35.2401",
    longitude = "24.8093"
)

/**
 * The [City] object for London, United Kingdom.
 */
val LONDON: City = City(
    name = "London",
    country = "United Kingdom",
    latitude = "51.509865",
    longitude = "-0.118092"
)

/**
 * The [City] object for Paris, France.
 */
val PARIS: City = City(
    name = "Paris",
    country = "France",
    latitude = "48.864716",
    longitude = "2.349014"
)

/**
 * The [List] of all of the [City] objects in our dataset.
 */
val listCities: List<City> = listOf(
    ROME, GRANADA, WASHINGTONDC, BARCELONA, CRETE, KHUMBUVALLEY, BIGSUR, BALI,
    ASPEN, MADRID, NAPLES, DALLAS, CORDOBA, BIGSUR, MALDIVAS, LONDON, PARIS
)
