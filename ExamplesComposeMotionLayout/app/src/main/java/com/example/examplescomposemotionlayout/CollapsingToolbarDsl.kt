 ...
 *  `top` to the `anchor` of [ConstrainedLayoutReference] `image` with a margin of `16.dp`, linking
 *  its `start` to the `start` of [ConstrainedLayoutReference] `image` with a margin of `16.dp`,
 *  and setting its `alpha` to `0f`.
 ...
 * [repeat] 5 times composing a [Text] whose `text` is 222 words of the nonsense produced by
 ...
 *
 * We initialize our [Float] variable `gap` to difference of the pixel value of [Dp] variable `big`
 * and [Dp] variable `small`. We initialize our [Float] variable `progress` to the minimum of
 * `scroll.value` divided by `gap`, and `1f`.
 *
 * We compose a [MotionLayout] whose `modifier` is a [Modifier.fillMaxSize] and whose `motionScene`
 * is our [MotionScene] variable `scene`, and whose `progress` is our [Float] variable `progress`.
 * In the [MotionLayoutScope] `content` composable lambda argument:
 ...
 *  the [MotionLayoutScope.customColor] whose `id` is the [ConstrainedLayoutReference] "image" and
 *  whose `name` is "cover". The `painter` of the [Image] is the [painterResource] whose `id` is the
 *  `jpg` with resource ID `R.drawable.bridge`. The `contentDescription` of the [Image] is `null`.
 *  The `contentScale` of the [Image] is [ContentScale.Crop].
 *  - We compose an [Image] whose `modifier` is a [Modifier.layoutId] whose `layoutId` is the
