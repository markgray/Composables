 ...
 * [ExploreModel] that corresponds to the `cityName` of the item clicked in [MainActivity]'s
 * UI.
 *
 * @param destinationsRepository the [DestinationsRepository] singleton injected by Hilt that we
 * should use in order to search its [List] of [ExploreModel] using its method
 * [DestinationsRepository.getDestination] for the [ExploreModel] whose [City.name]
 * matches the one passed as an extra in the [Intent] that launched [DetailsActivity].
...
class DetailsViewModel @Inject constructor(
     ...
     * [destinationsRepository] to search for the [ExploreModel] whose [City.name] matches the
     * [String] passed as an extra under the key [KEY_ARG_DETAILS_CITY_NAME] in the [Intent] that
     * launched [DetailsActivity]. If it succeeds it returns a [Result.Success] containing that
     * [ExploreModel], and if it fails it returns an [Result.Error] of [IllegalArgumentException]:
     * "City doesn't exist".
     */
    val cityDetails: Result<ExploreModel>
        get() {
            val destination: ExploreModel? = destinationsRepository.getDestination(cityName)
